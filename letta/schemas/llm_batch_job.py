...
class LLMBatchItem(LLMBatchItemBase, validate_assignment=True):
    ...
    request_status: JobStatus = Field(
        ...,
        description="The current status of the batch item request (e.g., PENDING, DONE, ERROR).",
    )
    ...
    batch_request_result: Optional[Union[BetaMessageBatchIndividualResponse]] = Field(
        None,
        description="The raw response received from the LLM provider for this item.",
    )
...
class LLMBatchJob(OrmMetadataBase, validate_assignment=True):
    ...
    status: JobStatus = Field(
        ...,
        description="The current status of the batch (e.g., created, in_progress, done).",
    )
    llm_provider: ProviderType = Field(
        ...,
        description="The LLM provider used for the batch (e.g., anthropic, openai).",
    )
    ...
    latest_polling_response: Optional[Union[BetaMessageBatch]] = Field(
        None,
        description="The most recent polling response received from the LLM provider.",
    )
    last_polled_at: Optional[datetime] = Field(
        None,
        description="The timestamp of the last polling check for the batch status.",
    )
